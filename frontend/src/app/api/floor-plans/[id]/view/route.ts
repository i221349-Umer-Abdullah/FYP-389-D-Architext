...
export async function GET(_request: Request, { params }: ViewRouteContext) {
  ...
  return new Response(download.stream, {
    headers: {
      ...
      "Content-Disposition": `inline; filename="${contentDispositionFileName(download.version.originalName)}"`,
    },
  });
}
